 ...
 */
export type WeatherKind = 'clear' | 'fair' | 'cloudy' | 'rain' | 'storm' | 'gale' | 'squall' | 'fog';
...
export const WEATHER: Record<WeatherKind, WeatherParams> = {
  ...
  // white squall: comes on in seconds, a hard blow in savage gusts, the sea whipped white, huge seas, a
  // deluge; over as quickly (see Weather.update / randomSpell)
  squall: { cloudCoverage: 0.85, cloudDensity: 1.7, cloudBase: 420, cloudTop: 5200, cloudType: 0.9, overcast: 0.82, rain: 1.0, lightning: 1.5, wind: 23, gustiness: 0.6, waves: 9, chop: 2.4, fog: 0.0065, whitecaps: 1 },
...
export const WEATHER_NAMES: Record<WeatherKind, string> = {
  clear: 'bezchmurnie', fair: 'małe zachmurzenie', cloudy: 'pochmurno', rain: 'deszcz', storm: 'burza', gale: 'sztorm', squall: 'biały szkwał', fog: 'mgła',
};
...
const ALIASES: Record<string, WeatherKind> = {
  ...
  squall: 'squall', szkwal: 'squall', 'szkwał': 'squall', 'bialy-szkwal': 'squall', 'white-squall': 'squall',
...
const NEXT: Record<WeatherKind, WeatherKind[]> = {
  ...
  // (a squall is rare: only out of heavy weather, one chance in several)
  storm: ['rain', 'gale', 'rain', 'gale', 'squall'],
  gale: ['storm', 'rain', 'storm', 'squall'],
  squall: ['storm', 'rain'],
...
export class Weather {
  ...
  private randomSpell(): number {
    // real seconds a weather spell lasts in auto mode; a squall blows itself out in a minute or so
    return this.kind === 'squall' ? 45 + this.rnd() * 35 : 80 + this.rnd() * 110;
  }
  ...
  cycle(): WeatherKind {
    const order: WeatherKind[] = ['clear', 'fair', 'cloudy', 'rain', 'storm', 'gale', 'squall', 'fog'];
  ...
  update(dt: number): void {
    ...
    // ease every parameter toward the target preset (~20 s time constant; a squall strikes in a few seconds)
    const k = 1 - Math.exp(-dt / (this.kind === 'squall' ? 5 : 20));
